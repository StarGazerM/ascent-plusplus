...
// DD backend extra derives: Ord/PartialOrd + Serialize/Deserialize are
// required by `ExchangeData` on tuple field types. These are additive
// (batch doesn't need them but isn't broken by them), so we'd upstream
// them in a PR rather than maintaining a fork indefinitely.
#[derive(Clone, PartialEq, Eq, Debug, Hash, PartialOrd, Ord, serde::Serialize, serde::Deserialize)]
...
fn test_ascent_run_tc_generic() {
   fn compute_tc<
      TNode: Clone + Hash + Eq + Ord + Sync + Send + serde::Serialize + serde::de::DeserializeOwned + ::std::fmt::Debug + 'static,
   >(r: &[(TNode, TNode)]) -> Vec<(TNode, TNode)> {
      ascent_run_m_par! {
         struct TC<TNode: Clone + Hash + Eq + Ord + Sync + Send + serde::Serialize + serde::de::DeserializeOwned + ::std::fmt::Debug + 'static>;
...
fn test_ascent_tc_generic() {
   ascent! {
      struct TC<TNode: Clone + Hash + Eq + Ord + serde::Serialize + serde::de::DeserializeOwned + ::std::fmt::Debug + 'static>;
      relation tc(TNode, TNode);
...
fn test_ascent_run_explicit_decl() {
   fn compute_tc<
      TNode: Clone + Eq + Hash + Ord + serde::Serialize + serde::de::DeserializeOwned + ::std::fmt::Debug + 'static,
   >(edges: &[(TNode, TNode)]) -> Vec<(TNode, TNode)> {
      ascent_run! {
         struct TC<TNode> where TNode: Clone + Eq + Hash + Ord + serde::Serialize + serde::de::DeserializeOwned + ::std::fmt::Debug + 'static;
