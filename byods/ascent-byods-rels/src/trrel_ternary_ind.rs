...
use crate::trrel_binary_ind::{TrRelIndCommon, TrRelInd0, TrRelInd1};
use crate::trrel_binary::MyHashSetIter;
use crate::utils::{hash_one, AltHashSet, AltHashSetIter};


#[derive(DerefMut, Deref)]
pub struct TrRel2IndCommonWrapper<const HAS_REVERSE_MAP1: bool, const HAS_REVERSE_MAP2: bool, T0: Clone + Hash + Eq, T1: Clone + Hash + Eq>(
...
impl<const HAS_REVERSE_MAP1: bool, const HAS_REVERSE_MAP2: bool, T0: Clone + Hash + Eq, T1: Clone + Hash + Eq> 
Default for TrRel2IndCommonWrapper<HAS_REVERSE_MAP1, HAS_REVERSE_MAP2, T0, T1> {
   fn default() -> Self {
      ...
      for (k, mut delta_trrel) in delta.map.drain() {
         ...
         match total.map.entry(k.clone()) {
            hashbrown::hash_map::Entry::Occupied(mut total_entry) => {
               TrRelIndCommon::merge_delta_to_total_new_to_delta(&mut new_trrel, &mut delta_trrel, total_entry.get_mut());
               if !delta_trrel.is_empty() {
                  new_delta_map.insert(k, delta_trrel);
               }
            },
            hashbrown::hash_map::Entry::Vacant(total_vacant_entry) => {
               let mut new_total = TrRelIndCommon::Old{ rel: Default::default(), anti_reflexive: delta_trrel.anti_reflexive()};
      ...
      for (k, mut new_trrel) in new.map.drain() {
         ...
         match total.map.entry(k.clone()) {
            ...
            hashbrown::hash_map::Entry::Vacant(_) => {
               TrRelIndCommon::merge_delta_to_total_new_to_delta(&mut new_trrel, &mut new_delta, &mut Default::default());
               new_delta_map.insert(k, new_delta);
            },
         }
      }
...
pub struct TrRel2Ind0<'a, T0: Clone + Hash + Eq, T1: Clone + Hash + Eq>(
   &'a TrRel2IndCommon<T0, T1>
);
...
pub struct TrRel2Ind0_1<'a, T0: Clone + Hash + Eq, T1: Clone + Hash + Eq>(
   &'a TrRel2IndCommon<T0, T1>
);
...
impl<'a, T0: Clone + Hash + Eq, T1: Clone + Hash + Eq> RelIndexReadAll<'a> for TrRel2Ind0_1<'a, T0, T1> {
   ...
   fn iter_all(&'a self) -> Self::AllIteratorType {
      Box::new(self.0.map.iter().flat_map(|(x0, v)| v.rel().map.iter().map(move |(x1, x2_set)| {
...
pub struct TrRel2Ind0_2<'a, T0: Clone + Hash + Eq, T1: Clone + Hash + Eq>(
   &'a TrRel2IndCommon<T0, T1>
);
...
impl<'a, T0: Clone + Hash + Eq, T1: Clone + Hash + Eq> RelIndexReadAll<'a> for TrRel2Ind0_2<'a, T0, T1> {
   ...
   fn iter_all(&'a self) -> Self::AllIteratorType {
      Box::new(self.0.map.iter().flat_map(|(x0, v)| v.rel().reverse_map.iter().map(move |(x1, x2_set)| {
         let iter: Self::ValueIteratorType = x2_set.iter().map(|x2| (x2, ));
         ((x0, x1), iter) 
      })))
   }
...
pub struct TrRel2Ind1<'a, T0: Clone + Hash + Eq, T1: Clone + Hash + Eq>(
   &'a TrRel2IndCommon<T0, T1>
);
...
impl<'a, T0: Clone + Hash + Eq, T1: Clone + Hash + Eq> TrRel2Ind1<'a, T0, T1> {
   fn get(&'a self, x1: &T1) -> Option<<Self as RelIndexRead<'a>>::IteratorType> {
      ...
      let res = move || x0s.iter().filter_map(move |x0| {
         Some(self.0.map.get(x0).unwrap().rel().map.get(x1)?.iter().map(move |x2| (x0, x2)))
      }).flatten();
...
pub struct TrRel2Ind2<'a, T0: Clone + Hash + Eq, T1: Clone + Hash + Eq>(
   &'a TrRel2IndCommon<T0, T1>
);
...
impl<'a, T0: Clone + Hash + Eq, T1: Clone + Hash + Eq> TrRel2Ind2<'a, T0, T1> {
   fn get(&'a self, x2: &T1) -> Option<<Self as RelIndexRead<'a>>::IteratorType> {
      ...
      let res = move || x0s.iter().flat_map(move |x0| {
...
pub struct TrRel2Ind1_2<'a, T0: Clone + Hash + Eq, T1: Clone + Hash + Eq>(
   &'a TrRel2IndCommon<T0, T1>
);
...
pub struct TrRel2IndNone<'a, T0: Clone + Hash + Eq, T1: Clone + Hash + Eq>(
   &'a TrRel2IndCommon<T0, T1>
);
...
impl<'a, T0: Clone + Hash + Eq, T1: Clone + Hash + Eq> RelIndexRead<'a> for TrRel2IndNone<'a, T0, T1> {
   ...
   fn index_get(&'a self, (): &Self::Key) -> Option<Self::IteratorType> {
      let res = || self.0.map.iter().flat_map(|(x0, trrel)| {
...
#[repr(transparent)]
pub struct TrRel2IndFull<'a, T0: Clone + Hash + Eq, T1: Clone + Hash + Eq>(
   &'a TrRel2IndCommon<T0, T1>
);
...
impl<'a, T0: Clone + Hash + Eq, T1: Clone + Hash + Eq> RelIndexReadAll<'a> for TrRel2IndFull<'a, T0, T1> {
   ...
   fn iter_all(&'a self) -> Self::AllIteratorType {
      let iter = self.0.map.iter().flat_map(|(x0, trrel)| {
         trrel.rel().iter_all().map(move |(x1, x2)| (x0, x1, x2))
      });
...
pub struct TrRel2IndFullWrite<'a, T0: Clone + Hash + Eq, T1: Clone + Hash + Eq>(
   &'a mut TrRel2IndCommon<T0, T1>
);
...
where Rel: std::ops::DerefMut<Target = TrRel2IndCommon<T0, T1>>,
{
   type RelIndex<'a>  = TrRel2IndFull<'a,T0,T1>where Self:'a,Rel:'a;
   #[inline(always)]
   fn to_rel_index<'a>(&'a self, rel: &'a Rel) -> Self::RelIndex<'a> { TrRel2IndFull(rel.deref()) }
   
   type RelIndexWrite<'a>  = TrRel2IndFullWrite<'a, T0, T1>where Self:'a,Rel:'a;
   #[inline(always)]
