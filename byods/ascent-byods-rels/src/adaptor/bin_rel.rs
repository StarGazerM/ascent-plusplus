...
pub trait ByodsBinRel: RelIndexMerge + Default {
   ...
   type Ind1AllIterValsIter<'a>: Iterator<Item = &'a Self::T0>
   where Self: 'a;
   ...
   fn is_empty(&self) -> bool { self.iter_all().next().is_none() }
}
...
impl<'a, TBinRel: ByodsBinRel> RelFullIndexRead<'a> for ByodsBinRelInd0_1<'a, TBinRel> {
   ...
   fn contains_key(&'a self, key: &Self::Key) -> bool { self.0.contains(&key.0, &key.1) }
}
...
where Rel: ByodsBinRel<T0 = T0, T1 = T1>
{
   ...
   type RelIndexWrite<'a>
      = ByodsBinRelInd0_1Write<'a, Rel>
   where
      Self: 'a,
      Rel: 'a;
   fn to_rel_index_write<'a>(&'a mut self, rel: &'a mut Rel) -> Self::RelIndexWrite<'a> { ByodsBinRelInd0_1Write(rel) }
}
