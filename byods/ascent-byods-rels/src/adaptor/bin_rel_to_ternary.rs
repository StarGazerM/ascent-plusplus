   ...
   TBinRel: ByodsBinRel<T0 = T1, T1 = T2>,
{
   ...
   fn iter_all(&'a self) -> impl Iterator<Item = (Self::Key, impl Iterator<Item = Self::Value> + 'a)> + 'a {
      self.0.map.iter().map(|(k, v)| ((k,), v.iter_all()))
   }
}
   ...
   TBinRel: ByodsBinRel<T0 = T1, T1 = T2>,
{
   ...
   fn iter_all(&'a self) -> impl Iterator<Item = (Self::Key, impl Iterator<Item = Self::Value> + 'a)> + 'a {
      self.0.map.iter().flat_map(|(x0, v)| {
         v.ind0_iter_all().map(move |(x1, x2s)| {
            ((x0, x1), x2s.map(|x2| (x2,)))
         })
      })
   }
   ...
   TBinRel: ByodsBinRel<T0 = T1, T1 = T2>,
{
   ...
   fn index_get(&'a self, key: &Self::Key) -> Option<impl Iterator<Item = Self::Value> + Clone + 'a> {
      let trrel = self.0.map.get(&key.0)?;
      let res = trrel.ind0_index_get(&key.1)?.map(|x| (x,));
      Some(res)
   }
   ...
   TBinRel: ByodsBinRel<T0 = T1, T1 = T2>,
{
   ...
   fn iter_all(&'a self) -> impl Iterator<Item = (Self::Key, impl Iterator<Item = Self::Value> + 'a)> + 'a {
      self.0.map.iter().flat_map(|(x0, v)| {
         v.ind1_iter_all().map(move |(x2, x1s)| {
            let iter = x1s.map(|x2| (x2,));
            ((x0, x2), iter)
         })
      })
   }
   ...
   TBinRel: ByodsBinRel<T0 = T1, T1 = T2>,
{
   ...
   fn index_get(&'a self, key: &Self::Key) -> Option<impl Iterator<Item = Self::Value> + Clone + 'a> {
      let trrel = self.0.map.get(&key.0)?;
      let res = trrel.ind1_index_get(&key.1)?.map(|x| (x,));
      Some(res)
   }
...
where
   ...
   TBinRel: ByodsBinRel<T0 = T1, T1 = T2>,
{
   fn get(&'a self, x1: &T1) -> Option<impl Iterator<Item = (&'a T0, &'a T2)> + Clone + 'a> {
      let (x1, x0s) = self.0.reverse_map1.as_ref().unwrap().get_key_value(x1)?;
      let res = move || {
         x0s.iter()
            .filter_map(move |x0| Some(self.0.map.get(x0).unwrap().ind0_index_get(x1)?.map(move |x2| (x0, x2))))
            .flatten()
      };
...
where
   ...
   TBinRel: ByodsBinRel<T0 = T1, T1 = T2>,
{
   fn get(&'a self, x2: &T2) -> Option<impl Iterator<Item = (&'a T0, &'a T1)> + Clone + 'a> {
      let (x2, x0s) = self.0.reverse_map2.as_ref().unwrap().get_key_value(x2)?;
      let res = move || {
         x0s.iter()
            .filter_map(move |x0| Some(self.0.map.get(x0).unwrap().ind1_index_get(x2)?.map(move |x1| (x0, x1))))
            .flatten()
      };
      Some(IteratorFromDyn::new(res))
   }
   ...
   fn iter_all(&'a self) -> impl Iterator<Item = (Self::Key, impl Iterator<Item = Self::Value> + 'a)> + 'a {
      self.0.reverse_map1.as_ref().unwrap().iter().flat_map(move |(x1, x0s_for_x1)| {
         self.0.reverse_map2.as_ref().unwrap().iter().map(move |(x2, x0s_for_x2)| {
            ((x1, x2), x0s_for_x1.intersection(x0s_for_x2).filter(|&x0| self.0.map.get(x0).unwrap().contains(x1, x2)).map(|x0| (x0,)))
         })
      })
   }
}
   ...
   TBinRel: ByodsBinRel<T0 = T1, T1 = T2>,
{
   ...
   fn index_get(&'a self, (x1, x2): &Self::Key) -> Option<impl Iterator<Item = Self::Value> + Clone + 'a> {
      ...
      let res =
         || x1_map.intersection(x2_map).filter(|&x0| self.0.map.get(x0).unwrap().contains(x1, x2)).map(|x0| (x0,));
      Some(IteratorFromDyn::new(res))
   }
   ...
   TBinRel: ByodsBinRel<T0 = T1, T1 = T2>,
{
   ...
   fn index_get(&'a self, (): &Self::Key) -> Option<impl Iterator<Item = Self::Value> + Clone + 'a> {
      let res = || self.0.map.iter().flat_map(|(x0, rel)| rel.iter_all().map(move |(x1, x2)| (x0, x1, x2)));
      Some(IteratorFromDyn::new(res))
   }
   ...
   TBinRel: ByodsBinRel<T0 = T1, T1 = T2>,
{
   ...
   fn iter_all(&'a self) -> impl Iterator<Item = (Self::Key, impl Iterator<Item = Self::Value> + 'a)> + 'a {
      self.0.map.iter().flat_map(|(x0, rel)| rel.iter_all().map(move |(x1, x2)| (x0, x1, x2))).map(|t| (t, once(())))
   }
}
...
impl<const HAS_REVERSE_MAP1: bool, const HAS_REVERSE_MAP2: bool, T0, T1, T2, TBinRel> RelIndexMerge
   for BinRelToTernaryWrapper<HAS_REVERSE_MAP1, HAS_REVERSE_MAP2, T0, T1, T2, TBinRel>
where
   T0: Clone + Hash + Eq,
   T1: Clone + Hash + Eq,
   T2: Clone + Hash + Eq,
   TBinRel: ByodsBinRel<T0 = T1, T1 = T2>,
{
   fn move_index_contents(from: &mut Self, to: &mut Self) { RelIndexMerge::move_index_contents(&mut from.0, &mut to.0) }
   fn merge_delta_to_total_new_to_delta(new: &mut Self, delta: &mut Self, total: &mut Self) {
      RelIndexMerge::merge_delta_to_total_new_to_delta(&mut new.0, &mut delta.0, &mut total.0)
   }
}

impl<const HAS_REVERSE_MAP1: bool, const HAS_REVERSE_MAP2: bool, T0, T1, T2, TBinRel> Default
   for BinRelToTernaryWrapper<HAS_REVERSE_MAP1, HAS_REVERSE_MAP2, T0, T1, T2, TBinRel>
where
   T0: Clone + Hash + Eq,
   T1: Clone + Hash + Eq,
...
where
   T0: Clone + Hash + Eq,
   T1: Clone + Hash + Eq,
   T2: Clone + Hash + Eq, //, TBinRel: ByodsBinRel<T0 = T1, T1 = T2>
{
   fn default() -> Self { Self(PhantomData) }
}

impl<T0, T1, T2, TBinRel, const HAS_REVERSE_MAP1: bool, const HAS_REVERSE_MAP2: bool>
   ToRelIndex<BinRelToTernaryWrapper<HAS_REVERSE_MAP1, HAS_REVERSE_MAP2, T0, T1, T2, TBinRel>>
   for ToBinRelToTernaryInd0_1_2<T0, T1, T2>
where
   T0: Clone + Hash + Eq,
   T1: Clone + Hash + Eq,
   T2: Clone + Hash + Eq,
...
{
   type RelIndex<'a>
      = BinRelToTernaryInd0_1_2<'a, T0, T1, T2, TBinRel>
   where
      Self: 'a,
      BinRelToTernaryWrapper<HAS_REVERSE_MAP1, HAS_REVERSE_MAP2, T0, T1, T2, TBinRel>: 'a;
   #[inline(always)]
   fn to_rel_index<'a>(
      &'a self, rel: &'a BinRelToTernaryWrapper<HAS_REVERSE_MAP1, HAS_REVERSE_MAP2, T0, T1, T2, TBinRel>,
   ) -> Self::RelIndex<'a> {
      BinRelToTernaryInd0_1_2(&rel.0)
   }

   type RelIndexWrite<'a>
      = BinRelToTernaryInd0_1_2Write<'a, T0, T1, T2, TBinRel>
   where
      Self: 'a,
      BinRelToTernaryWrapper<HAS_REVERSE_MAP1, HAS_REVERSE_MAP2, T0, T1, T2, TBinRel>: 'a;
   #[inline(always)]
   fn to_rel_index_write<'a>(
      &'a mut self, rel: &'a mut BinRelToTernaryWrapper<HAS_REVERSE_MAP1, HAS_REVERSE_MAP2, T0, T1, T2, TBinRel>,
