...
#[derive(Clone)]
pub struct EqRel2IndWrapper<const WITH_REVERSE: bool, T0: Clone + Hash + Eq, T1: Clone + Hash + Eq>(EqRel2IndCommon<T0, T1>);
...
impl<const WITH_REVERSE: bool, T0: Clone + Hash + Eq, T1: Clone + Hash + Eq> RelIndexMerge for EqRel2IndWrapper<WITH_REVERSE, T0, T1> {
...
impl<const WITH_REVERSE: bool, T0: Clone + Hash + Eq, T1: Clone + Hash + Eq> Default for EqRel2IndWrapper<WITH_REVERSE, T0, T1> {
   fn default() -> Self {
...
where T0: Clone + Hash + Eq, T1: Clone + Hash + Eq 
{
...
impl<T0: Clone + Hash + Eq, T1: Clone + Hash + Eq> EqRel2IndCommon<T0, T1> {
   fn insert_into_reverse_map(&mut self, key: &(T0, T1, T1)) {
      if let Some(reverse_map) = &mut self.reverse_map {
         reverse_map.raw_entry_mut().from_key(&key.1).or_insert_with(|| (key.1.clone(), Default::default())).1.insert(key.0.clone());
         reverse_map.raw_entry_mut().from_key(&key.2).or_insert_with(|| (key.2.clone(), Default::default())).1.insert(key.0.clone());
      }
   }
}
...
impl<T0: Clone + Hash + Eq, T1: Clone + Hash + Eq> RelIndexMerge for EqRel2IndCommon<T0, T1> {
   ...
   fn merge_delta_to_total_new_to_delta(new: &mut Self, delta: &mut Self, total: &mut Self) {
      ...
      if delta.reverse_map.is_some() {
         crate::utils::move_hash_map_of_hash_set_contents(&mut delta.reverse_map.as_mut().unwrap(), total.reverse_map.as_mut().unwrap());
         crate::utils::move_hash_map_of_hash_set_contents(new.reverse_map.as_mut().unwrap(), delta.reverse_map.as_mut().unwrap());
      }
   }
}
...
impl<T0: Clone + Hash + Eq, T1: Clone + Hash + Eq> EqRel2IndCommon<T0, T1> {
   pub(crate) fn iter_all_added(& self) -> impl Iterator<Item = (&T0, &T1, &T1)> {
      self.map.iter().flat_map(|(t0, eqrel)| {
         eqrel.iter_all_added().map(move |(t1, t2)| (t0, t1, t2))
      })
   }
}

pub struct EqRel2Ind0_1<'a, T0: Clone + Hash + Eq, T1: Clone + Hash + Eq>(
   &'a EqRel2IndCommon<T0, T1>
);
...
impl<'a, T0: Clone + Hash + Eq, T1: Clone + Hash + Eq> RelIndexRead<'a> for EqRel2Ind0_1<'a, T0, T1> {
   ...
   fn len(&self) -> usize {
      let sample_size = 4;
      let (count, sum) = self.0.map.values().take(sample_size).map(|eqrel| eqrel.combined.elem_ids.len()).fold((0, 0), |(c, s), x| (c + 1, s + x));
...
impl<'a, T0: Clone + Hash + Eq, T1: Clone + Hash + Eq> RelIndexReadAll<'a> for EqRel2Ind0_1<'a, T0, T1> {
   ...
   fn iter_all(&'a self) -> Self::AllIteratorType {
      Box::new(self.0.map.iter().flat_map(|(t0, eqrel)| {
         eqrel.iter_all_added().map(move |(t1, t2)| ((t0, t1), std::iter::once((t2,)) ))
      }))
   }
}

pub struct EqRel2Ind0<'a, T0: Clone + Hash + Eq, T1: Clone + Hash + Eq>(
   &'a EqRel2IndCommon<T0, T1>
);
...
impl<'a, T0: Clone + Hash + Eq, T1: Clone + Hash + Eq> RelIndexReadAll<'a> for EqRel2Ind0<'a, T0, T1> {
   ...
   fn iter_all(&'a self) -> Self::AllIteratorType {
      Box::new(self.0.map.iter().map(|(t, eqrel)| ( ref_to_singleton_tuple_ref(t), IteratorFromDyn::new(|| eqrel.iter_all_added()))))
   }
}
...
pub struct EqRel2Ind1<'a, T0: Clone + Hash + Eq, T1: Clone + Hash + Eq>(
   &'a EqRel2IndCommon<T0, T1>
);
...
impl<'a, T0: Clone + Hash + Eq, T1: Clone + Hash + Eq> RelIndexReadAll<'a> for EqRel2Ind1<'a, T0, T1> {
   ...
   fn iter_all(&'a self) -> Self::AllIteratorType {
      Box::new(self.0.reverse_map.as_ref().unwrap().keys().map(ref_to_singleton_tuple_ref).map(|k| (k, self.index_get(k).unwrap())))
   }
}
...
impl<'a, T0: Clone + Hash + Eq, T1: Clone + Hash + Eq> RelIndexRead<'a> for EqRel2Ind1<'a, T0, T1> {
   ...
   fn index_get(&'a self, key: &Self::Key) -> Option<Self::IteratorType> {
      ...
      let t1 = key.0.clone();
      let res = move || t0s.iter().zip(std::iter::repeat(t1.clone())).flat_map(move |(t0, t1)| self.0.map.get(t0).unwrap().set_of_added(&t1).into_iter().flatten().map(move |t2| (t0, t2)));
      Some(IteratorFromDyn::new(res))
   }

   fn len(&self) -> usize {
      self.0.reverse_map.as_ref().unwrap().len()
   }
}

pub struct EqRel2Ind1_2<'a, T0: Clone + Hash + Eq, T1: Clone + Hash + Eq>(
   &'a EqRel2IndCommon<T0, T1>
);
...
impl<'a, T0: Clone + Hash + Eq, T1: Clone + Hash + Eq> RelIndexReadAll<'a> for EqRel2Ind1_2<'a, T0, T1> {
   ...
   type ValueIteratorType = Map<hashbrown::hash_set::Intersection<'a, T0, BuildHasherDefault<FxHasher>>, for<'aa> fn(&'aa T0) -> (&T0,)>;

   type AllIteratorType = Box<dyn Iterator<Item = (Self::Key, Self::ValueIteratorType)> + 'a>;

   fn iter_all(&'a self) -> Self::AllIteratorType {
      let res = self.0.reverse_map.as_ref().unwrap().iter()
...
impl<'a, T0: Clone + Hash + Eq, T1: Clone + Hash + Eq> RelIndexRead<'a> for EqRel2Ind1_2<'a, T0, T1> {
   ...
   fn index_get(&'a self, key: &Self::Key) -> Option<Self::IteratorType> {
      ...
      let key = key.clone();
      let res = move || t0s.iter().zip(std::iter::repeat(key.clone()))
...
{
   type RelIndex<'a> = EqRel2IndFull<'a, T0, T1> where Self: 'a, Rel: 'a;
   fn to_rel_index<'a>(&'a self, rel: &'a Rel) -> Self::RelIndex<'a> { EqRel2IndFull(rel.to_eq_rel2_ind_common()) }

   type RelIndexWrite<'a> = &'a mut EqRel2IndCommon<T0, T1> where Self: 'a, Rel: 'a;
...
impl<'a, T0: Clone + Hash + Eq, T1: Clone + Hash + Eq> RelIndexRead<'a> for EqRel2IndFull<'a, T0, T1> {
   ...
   fn index_get(&'a self, key: &Self::Key) -> Option<Self::IteratorType> {
      if self.contains_key(key) { Some(std::iter::once(&())) } 
      else { None }
   }
...
impl<'a, T0: Clone + Hash + Eq, T1: Clone + Hash + Eq> RelFullIndexRead<'a> for EqRel2IndFull<'a, T0, T1> {
   ...
   fn contains_key(&'a self, (t0, t1, t2): &Self::Key) -> bool {
      if let Some(eqrel) = self.0.map.get(t0) { eqrel.added_contains(t1, t2) } 
      else { false }
   }
}

pub struct EqRel2IndNone<'a, T0: Clone + Hash + Eq, T1: Clone + Hash + Eq>(
   &'a EqRel2IndCommon<T0, T1>
);
